...
const FloatingPaths = memo(function FloatingPaths({
  ...
  const colors = { ...defaultGradient, ...gradientColors };
      ...
      <svg
        ...
        <defs>
          <linearGradient id="sharedGradient" x1="0%" x2="100%" y1="0%" y2="0%">
            <stop offset="0%" stopColor={colors.primary} stopOpacity={0.5 * opacity} />
            <stop offset="50%" stopColor={colors.middle} stopOpacity={0.5 * opacity} />
            <stop offset="100%" stopColor={colors.accent} stopOpacity={0.5 * opacity} />
          </linearGradient>
        </defs>
...
const AnimatedTitle = memo(function AnimatedTitle({ title }: { title: string }) {
  return (
    <motion.h1
...
export default memo(function BackgroundPaths({
  ...
  return (
    <div
      className={`relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-transparent ${className}`}>
      <div className="absolute inset-0">
        <FloatingPaths position={1} gradientColors={gradientColors} opacity={opacity} />
      </div>
