...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className="w-fit group peer hidden text-sidebar-foreground md:block"
        data-state={state}
      ...
      /> */}
        <div
          className={cn(
            'h-svh w-[--sidebar-width] transition-[left,right,width] duration-200 ease-linear hidden md:flex relative z-10',
            // side === "left"
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-sm p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium  [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground',
        outline:
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Random width between 50 to 90% (lazy init avoids impure render per react-hooks/purity).
