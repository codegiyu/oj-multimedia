...
const ANSWERED_QUESTIONS_GRID_CLASS = 'grid md:grid-cols-2 gap-6';

function AnsweredQuestionCards({ questions }: { questions: AnsweredQuestion[] }) {
  return (
    <>
...
export const AnsweredQuestionsSection = ({
  questions,
  pagination = null,
  ...
  const grid = <AnsweredQuestionCards questions={itemsToShow} />;

  if (isBrowseList) {
    return (
      <ContentBrowseList pagination={pagination} gridClassName={ANSWERED_QUESTIONS_GRID_CLASS}>
        {grid}
      </ContentBrowseList>
    );
  }
  ...
  return (
    <SectionComp
      ...
      <div className={ANSWERED_QUESTIONS_GRID_CLASS}>{grid}</div>
