...
export const VideoSermonsSection = ({ sermons }: VideoSermonsSectionProps) => {
  ...
  const handleShare = async (e: React.MouseEvent, sermon: VideoSermon) => {
    e.preventDefault();
    e.stopPropagation();
    try {
      const url = `${window.location.origin}/community/sermons/${sermon.id}`;
      if (navigator.share) {
        await navigator.share({
          title: sermon.title,
          text: `Check out this sermon by ${sermon.pastor}`,
          url,
        });
        toast({
          title: 'Shared!',
    ...
    <section className="py-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button variant="ghost" className="gap-2 text-accent" asChild>
          <Link href="/community/sermons/video">
            View All Videos
