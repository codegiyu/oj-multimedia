...
const STATUS_OPTIONS: { value: 'draft' | 'published' | 'archived'; text: string }[] = [
  { value: 'draft', text: 'Draft' },
  { value: 'published', text: 'Published' },
  { value: 'archived', text: 'Archived' },
];

function getCategoryId(product: IMarketplaceProduct): string {
  return product.category?._id ?? '';
}

function getSubCategoryId(product: IMarketplaceProduct): string {
...
function getEditSubmitValidationMessages(
  values: EditProductValues,
  errors: Partial<Record<keyof EditProductValues | 'root', string[] | undefined>>,
  ...
  (
    ['name', 'price', 'description', 'category', 'subCategory', 'tagsString', 'status'] as const
  ).forEach(key => {
...
export function VendorEditProductPageClient({ productId }: { productId: string }) {
  const router = useRouter();
  const [product, setProduct] = useState<IMarketplaceProduct | null>(null);
  ...
  const {
    ...
    setFormValues,
    isValid,
  } = useForm<typeof editProductSchema>({
    ...
    async onSubmit(values: EditProductValues) {
      ...
      let payload: IVendorUpdateProductPayload = {
        name: values.name,
  ...
  // Sync form and variations when product loads
  useEffect(() => {
    if (!product) return;
    setFormValues({
      name: product.name,
      ...
      subCategory: getSubCategoryId(product),
      tagsString: product.tags?.length ? product.tags.join(', ') : '',
      price: product.price,
      inStock: product.inStock ?? true,
      status: product.status as 'draft' | 'published' | 'archived',
      isFeatured: product.isFeatured ?? false,
      imageUrl: product.images?.[0] ?? '',
      otherImages: (product.images?.slice(1, MAX_OTHER_IMAGES + 1) ?? []) as string[],
    });
    if (product.variationOptions?.length && product.variants?.length) {
      setHasVariations(true);
      setVariationOptions(
        product.variationOptions.map(o => ({ name: o.name, values: [...(o.values || [])] }))
      );
      setVariants(
  ...
  useEffect(() => {
    if (!variationsInitialized || !hasVariations || variationOptions.length === 0) return;
    if (skipNextRecomputeRef.current) {
      skipNextRecomputeRef.current = false;
      return;
    }
    recomputeVariants(variationOptions, variants);
  }, [hasVariations, variationOptions, variationsInitialized]);
  ...
  const statusOptions = STATUS_OPTIONS.map(opt => ({ text: opt.text, value: opt.value }));
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <DashboardPageHeader
        title="Edit product"
        description="Update listing details, images, and variants."
      />
      <Card className="p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <RegularSelect
            ...
            onSelectChange={v => onChange('status', v as 'draft' | 'published' | 'archived')}
          ...
          <div className="flex gap-2">
            <RegularBtn
              ...
              loading={submitting}
              onDisabledClick={() => {
                const messages = getEditSubmitValidationMessages(
                  formValues,
                  formErrors,
                  hasVariations,
                  variationOptions,
