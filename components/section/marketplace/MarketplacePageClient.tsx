            ...
            {categories.map((category, idx) => (
              <Card key={idx} className="p-6 text-center hover:shadow-lg transition-shadow group">
                <Link href={`/marketplace?category=${category.slug}`}>
                  <h3 className="font-semibold text-foreground mb-2 group-hover:text-[#2563EB] transition-colors">
                    {category.name}
                  </h3>
              ...
              <Card key={idx} className="group hover:shadow-lg transition-shadow">
                <CardContent className="p-6">
                  <Link href={`/marketplace/products/${product.slug}`}>
                    ...
                    <p className="text-lg font-bold text-[#2563EB] mb-4">{product.price}</p>
                    <div className="flex items-center gap-2">
                      <GhostBtn className="flex-1 text-sm text-[#2563EB] hover:text-[#3B82F6]">
                        <MessageCircle className="w-4 h-4 mr-1" />
                        Chat
                      </GhostBtn>
                      <RegularBtn
                        text="Buy"
                        className="flex-1 bg-[#2563EB] hover:bg-[#3B82F6] text-white text-sm"
                      />
                    </div>
                  </Link>
                </CardContent>
              ...
              <Card key={idx} className="group hover:shadow-lg transition-shadow">
                <CardContent className="p-6">
                  <Link href={`/marketplace/vendors/vendor-${idx + 1}`}>
                    <div className="w-20 h-20 rounded-full bg-[#2563EB]/10 mx-auto mb-4 flex items-center justify-center">
                      <Store className="w-10 h-10 text-[#2563EB]" />
                    </div>
                    <h3 className="text-xl font-bold text-foreground text-center mb-2 group-hover:text-[#2563EB] transition-colors">
                      Vendor Store {idx + 1}
                    </h3>
                    <p className="text-sm text-muted-foreground text-center mb-4">15 products</p>
                    <GhostBtn className="w-full text-center text-[#2563EB] hover:text-[#3B82F6]">
                      Visit Store
                    </GhostBtn>
                  </Link>
                </CardContent>
