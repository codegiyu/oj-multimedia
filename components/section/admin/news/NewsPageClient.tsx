...
export function NewsPageClient() {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      const { data, error } = await callApi('ADMIN_NEWS_LIST', {
      ...
      const items = (data as { news?: PublicNewsListItem[] })?.news ?? [];
  ...
  useEffect(() => {
    debounceRef.current = setTimeout(() => fetchNews(), SEARCH_DEBOUNCE_MS);
    return () => {
