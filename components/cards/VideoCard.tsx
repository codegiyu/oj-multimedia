...
interface VideoCardProps {
  ...
  duration: string;
  category: string;
  /** 'featured' | 'recent' = different layouts. 'recent' = horizontal row, small thumb, uploadedAt */
...
export const VideoCard = ({
  ...
  return (
    <motion.div
      whileHover={{ y: isFeaturedVariant ? -8 : -4 }}
      transition={{ duration: 0.3 }}
        ...
        {detailHref ? (
          <AppLink
            ...
            <FillImage
              ...
              className={`transition-transform duration-500 ${isFeaturedVariant ? 'group-hover:scale-110' : 'group-hover:scale-105'}`}
            />

            <div
              className={`absolute inset-0 flex items-center justify-center transition-opacity pointer-events-none ${isFeaturedVariant ? 'bg-gradient-to-t from-foreground/80 via-transparent to-transparent opacity-0 group-hover:opacity-100' : 'bg-foreground/20 opacity-0 group-hover:opacity-100'}`}>
              <span
                className={`flex items-center justify-center rounded-full bg-primary text-primary-foreground ${isFeaturedVariant ? 'h-10 w-10 shadow-glow' : 'h-12 w-12'}`}>
                <Play
                  className={`fill-current ${isFeaturedVariant ? 'w-4 h-4 ml-0.5' : 'w-6 h-6 ml-1'}`}
                  aria-hidden
                />
              </span>
            </div>
            ...
            <span
              className={`absolute font-medium rounded bg-foreground/80 pointer-events-none ${isFeaturedVariant ? 'bottom-3 right-3 text-xs px-2 py-1 rounded-md text-background' : 'bottom-2 right-2 flex items-center gap-1 text-xs px-2 py-1 rounded-md text-primary-foreground'}`}>
              {!isFeaturedVariant && <Clock className="w-3 h-3" />}
              {duration}
            </span>
